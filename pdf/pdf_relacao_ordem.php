 ...
 while($linha_resultado = mysqli_fetch_array($result))
            {
                ...
                $relatorio.="<tr class =''>";
                $relatorio.="<td class =''> {$linha_resultado['codigoOS']} </td>";
                $relatorio.="<td> {$linha_resultado['codigoProduto']} </td>";
                $relatorio.="<td> {$linha_resultado['nomeProduto']} </td>";

                $relatorio.="<td> {$linha_resultado['quantidadeProduzida']} </td>";
...
$dompdf->stream("relacaoordem.pdf",array("Attachment" => false));//true= download, false = navegador
